        ...
        try:
            ...
            for i in buttons_dict:
                if data_btn == i.get("callback_data", None):
                    #i['text']
                    markup_tpm.add(types.InlineKeyboardButton(text="Перейти за посиланням", url=
                    config_controller.LIST_POSTS[name]['urls'][url_ind]))
                    await db.add_user_event_by_tg_id(user_tg_id=self.user_id, event_name="joinFrom_"+name+"_"+str(url_ind))
                else:
