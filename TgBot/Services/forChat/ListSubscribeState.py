...
class ListSubscribeState(UserState):
    ...
    async def generate_stat(self):
        ...
        namefile, count = await obj_stat.get_file_name_and_count_statistic(self.start, self.end, self.user_id, by=self.typestat, filtr="joinFromNeed_"+str(
            config_controller.LIST_SUBSCRIBE[self.name_suscribe]['id']))
        ...
        if self.is_list:
            if data_btn == "/edit" and self.name_suscribe != None:
                ...
                return Response(text="Назва: " + data_btn +"\nUrl: " + config_controller.LIST_SUBSCRIBE[data_btn]['url'] + "\nID: " + str(
                    config_controller.LIST_SUBSCRIBE[data_btn]['id']), buttons=markups.generate_subscribe_semimenu())
