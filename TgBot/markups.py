...
def generate_post_menu(offset: int=0, max:int = 5):
    ...
    markup.add(types.InlineKeyboardButton(text="Статус розсилки", callback_data="/status_send"))
...
def generate_cancel_and_none():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(types.InlineKeyboardButton(text="Без посилань", callback_data="/none"))
    markup.add(types.InlineKeyboardButton(text="❌Відмінити❌", callback_data="/cancel"))
    return markup
