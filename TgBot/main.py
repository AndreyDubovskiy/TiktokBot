...
from Services.utils_for_api.forDownloadApi import static_download_api
import io
        ...
        if await is_subscribe(message.from_user.id):
            ...
            while(True):
                if iter_num > iter_max:
                    break
                try:
                    count_files = await static_download_api.send_download_request(message.text, str(chat_id))
                    if count_files == 0:
                        iter_num += 1
                        await asyncio.sleep(5)
                        continue
                    for i in range(count_files):
                        res = await static_download_api.get_file(str(chat_id))
                        buf = io.BytesIO()
                        async for chunk in res.aiter_bytes(chunk_size=1024 * 512):
                            if chunk:
                                buf.write(chunk)
                        buf.seek(0)
                        buf.name = str(chat_id) + "_" + str(i) + ext_map.get(res.headers["content-type"], "")
                        res_list.append((buf, res.headers["content-type"]))
                except:
                    iter_num += 1
                    await asyncio.sleep(5)
                    continue
            ...
            if len(res_list) == 0:
                await bot.reply_to(message, config_controller.CONTACT_HELP)
                return
            if ext_map[res_list[0][1]] == '.mp4':
                for i in res_list:
                    await bot.send_video(chat_id=message.chat.id, video=i[0], supports_streaming=True)
                if config_controller.IS_SEND_AFTERVIDEO:
                    await bot.send_message(chat_id=message.chat.id, text=config_controller.TEXT_AFTER_VIDEO, parse_mode="HTML")
            elif ext_map[res_list[0][1]] == '.jpg' or ext_map[res_list[0][1]] == '.png' or ext_map[res_list[0][1]] == '.webp':
                media_group = []
                music = res_list[-1][0]
                this = [i[0] for i in res_list[:-1]]
                for photo in this:
...
async def is_subscribe(chat_id):
    ...
    try:
        ...
        if not(chat_id in list_user_cheked):
            if not (await db.is_created_user(user_tg_id=chat_id)):
                await db.create_user(user_tg_id=chat_id, user_name="None")
        ...
        if chat_id in list_user_left:
            ...
            for i in list_user_unsubscribe[str(chat_id)]:
                await db.add_user_event_by_tg_id(user_tg_id=chat_id, event_name="joinFromNeed_"+str(i))
