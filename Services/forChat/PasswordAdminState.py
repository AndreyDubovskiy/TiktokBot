...
class PasswordAdminState(UserState):
    def __init__(self, user_id: str, user_chat_id: str, bot: AsyncTeleBot):
        super().__init__(user_id, user_chat_id, bot)
        self.is_login = False
    async def start_msg(self):
        self.is_login = True
        return Response(text="Введіть наступним повідомленням новий пароль для адміна", buttons=markups.generate_cancel())

    async def next_msg(self, message: str):
        if self.is_login:
            res = config_controller.change_password_admin(self.user_id, message)
            if res:
